...
def test_stdout_final_strategy_collects_output(test_db):
    ...
    artifacts = strategy.collect_artifacts(
        job_id=job_id, job_dir=job_dir, final_output=final_output, db_conn=test_db.conn
    )
...
def test_stdout_final_strategy_returns_empty_if_no_output(test_db):
    ...
    artifacts = strategy.collect_artifacts(
        job_id="test-job-2", job_dir=Path.cwd(), final_output=None, db_conn=test_db.conn
    )
...
def test_get_strategy_returns_composite():
    ...
    config = {
        "type": "composite",
        "strategies": [{"type": "stdout_final"}, {"type": "git_diff"}],
    }
