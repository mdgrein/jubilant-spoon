...
def _quota_wait(wait_until: float) -> None:
    ...
    while True:
        ...
        remaining = wait_until - time.time()
        if remaining > 0:
            print(f"[claude] Still waiting for quota reset… {remaining / 60:.1f} min left", flush=True)
        ...
        for raw_line in iter(proc.stdout.readline, ""):
            ...
            if etype == "rate_limit_event":
                ...
                if status == "rejected":
                    ...
                    quota_hit.append(_QuotaExhausted(float(raw_ts) if raw_ts is not None else None))
                ...
                if event.get("is_error") and not quota_hit:
                    ...
                    if any(kw in combined.lower() for kw in ("limit", "billing", "quota", "exceeded")):
...
def _retry_after_quota(prompt: str, model: str | None, session_id: str | None) -> tuple[str, int, int]:
    ...
    if session_id:
        ...
        text, quota_err, _, ti, to = _call_claude_once(resume_prompt, model, session_id=session_id)
        if quota_err:
            raise RuntimeError("Claude quota exhausted twice in a row; giving up.")
        if text:
            return text, ti, to
        print("[claude] --resume returned empty result; falling back to original prompt.", flush=True)
...
def call_claude(prompt: str, model: str | None = None) -> tuple[str, int, int]:
    ...
    text, quota_err, session_id, tokens_in, tokens_out = _call_claude_once(prompt, model)
    ...
    if reset_ts is None:
        ...
        print("[claude] No resetsAt in quota event; defaulting to 5 h wait.", flush=True)
