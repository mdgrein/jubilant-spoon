...
from dev_utils import classify_failure, parse_pytest_failures, format_failure_for_llm, get_test_code, get_test_code_by_name, extract_all_io_examples
...
def _initial_prompt(task: str, input_file_contents: dict[str, str], prior_code: str | None = None) -> str:
    prompt = (
...
def _retry_prompt(task: str, current_code: str, llm_feedback: str, input_file_contents: dict[str, str], is_format_failure: bool = False) -> str:
    base_prompt = (
        "You are a Python programmer. "
        "Output ONLY raw Python code. No explanations, no markdown, no code fences.\n\n"
    )
...
def _test_failure_prompt(task: str, current_code: str, pytest_output: str, input_file_contents: dict[str, str]) -> str:
...
def _single_test_failure_prompt(task: str, current_code: str, test_name: str, failure_traceback: str, test_code: str, input_file_contents: dict[str, str]) -> str:
...
def main():
    ...
    job_succeeded_in_loop = False # NEW: Track if the entire job succeeded and broke out of the loop

    # State for "one test at a time" mode
    test_todo_list = []
    current_todo_test = None
    all_tests_failure_count = 0 # How many times have we failed in the "all at once" mode?
    ...
    for attempt in range(start_attempt, MAX_ITERATIONS + 1):
        ...
        print(f"\n\n{'='*80}")
        print(f"  ITERATION {attempt}/{MAX_ITERATIONS}  |  Job: {job_id[:8]}  |  File: {filename}")
        print(f"{'='*80}\n")
        ...
        try:
            if last_failure is None:
                model_prompt = _with_warnings(_initial_prompt(task, input_file_contents, existing_code), current_warnings)
            elif last_failure == "ruff":
                model_prompt = _with_warnings(_retry_prompt(task, existing_code, llm_feedback, input_file_contents), current_warnings)
            elif last_failure == "format_error": # Special prompt for format failures
                model_prompt = _with_warnings(_retry_prompt(task, existing_code, llm_feedback, input_file_contents, is_format_failure=True), current_warnings)
                ...
                traceback = parsed_failure.traceback if parsed_failure else "No traceback found."
                ...
                if len(history) >= _STUCK_THRESHOLD and len(set(history)) == 1:
                    ...
                    if examples:
                        table = "\n".join(f"  {call} {outcome}" for call, outcome in examples)
                        stuck_msg += (
                ...
                if test_code.startswith("# [") and parsed_failure and parsed_failure.file_path and parsed_failure.line_number:
                    # Fallback: path+line extracted from pytest output (unreliable on Windows).
                    test_code = get_test_code(parsed_failure.file_path, parsed_failure.line_number, workspace)

                model_prompt = _with_warnings(
                    _single_test_failure_prompt(
                        task, existing_code, test_name, traceback, test_code, input_file_contents
                    ),
                    current_warnings,
                )
            else:  # 'pytest' (all at once)
                model_prompt = _with_warnings(_test_failure_prompt(task, existing_code, pytest_output, input_file_contents), current_warnings)
            ...
            try:
                ...
                db.log_action(job_id, attempt, {}, {"status": "timeout"}, raw_stdout="TIMEOUT")
                continue
            ...
            if not code.strip():
                ...
                try:
                    ...
                    print(f"[direct] LLM output is not valid Python code (parsing error): {e}")
                    llm_feedback = (
                        f"Your output must consist ONLY of valid raw Python code. "
            ...
            if test_path.exists():
                ...
                if rc == 0:
                    ...
                    summary = next(
                        (l for l in reversed(pytest_output.splitlines()) if "failed" in l.lower()),
                        "tests failed",
                    )
                    ...
                    elif current_failure_count > best_failure_count and best_code is not None:
                        print(
                    ...
                    if existing_code and last_failure in ("single_test", "pytest"):
                        ...
                        changed = [
                            l for l in difflib.unified_diff(old_lines, new_lines)
                    ...
                    if last_failure == "single_test":
                        if current_todo_test not in failures:
                            ...
                            stuck_tracker.pop(current_todo_test, None)  # reset stuck history for this test
                            action_results["test_fixed"] = current_todo_test
                            if test_todo_list:
                                 current_todo_test = test_todo_list.pop(0)
                                 print(f"[direct] Moving to next test in todo list: {current_todo_test}")
                                 last_failure = "single_test"
                            else:
                                 print("[direct] Todo list empty, but some tests still fail. Refreshing todo list.")
                                 test_todo_list = list(failures.keys())
                                 current_todo_test = test_todo_list.pop(0)
                                 last_failure = "single_test"
                        else:
                            print(f"[direct] Test {current_todo_test} STILL failing. Retrying it.")
                            last_failure = "single_test"
                    else:
    ...
    if not job_succeeded_in_loop:
        if last_failure == "format_error":
             fail_job(db, job_id, f"LLM output format consistently invalid after {MAX_ITERATIONS} attempts", tag="direct")
        elif last_failure == "ruff":
             fail_job(db, job_id, f"did not pass ruff checks after {MAX_ITERATIONS} iterations", tag="direct")
        elif last_failure == "pytest":
             fail_job(db, job_id, f"tests still failing after {MAX_ITERATIONS} iterations", tag="direct")
        else:
             fail_job(db, job_id, f"job failed after {MAX_ITERATIONS} iterations for unknown reason", tag="direct")
