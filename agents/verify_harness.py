...
def main():
    ...
    ts = db._timestamp()
    db.conn.execute("UPDATE jobs SET iteration = 1, updated_at = ? WHERE job_id = ?", (ts, job_id))
    db.conn.commit()
    ...
    iteration_log = f"[verify] pytest output (returncode={returncode}):\n{pytest_output}\n"
    ...
    if returncode >= 2:
        ...
        print(f"[verify] collection error (returncode {returncode}) — retry-able failure")
        ...
        if returncode == 0 and verdict != "PASS":
            failure_context = (
                f"VERIFIER CRITIQUE (tests passed but implementation is incorrect):\n{explanation}"
            )
        else:
            failure_context = (
                f"VERIFIER CRITIQUE:\n{explanation}\n\n"
    ...
    if verdict == "PASS":
        ...
        db.log_action(job_id, 1, action_llm_response, action_results, raw_stdout=iteration_log)
    ...
    print(f"[verify] REJECTED (dev attempt {attempt_num}/{MAX_DEV_RETRIES}): {explanation}")
