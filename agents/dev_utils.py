...
class FailureInfo:
    ...
    def to_hint(self) -> str:
        ...
        if self.category == FailureCategory.IMPORT_ERROR:
            return "TIP: There is an import error. Check your imports and file structure."
...
def classify_failure(test_output: str) -> FailureInfo:
    ...
    match = re.search(r"AttributeError: (?:module|object) '?.+'? has no attribute '(.+)'", test_output)
    if match:
        return FailureInfo(
...
def format_failure_for_llm(test_name: str, traceback: str, test_code: str | None = None) -> str:
    """
    ...
    match_did_not_raise = re.search(r"Failed: DID NOT RAISE <class '(.+)'>", error_detail)
    if match_did_not_raise:
        exc_type = match_did_not_raise.group(1).split('.')[-1] # Get short name like 'ValueError'
    ...
    if match_assert:
        ...
        if failing_code_line:
            ...
            return f"The code produced {actual} but it was expected to produce {expected}."
        ...
        for i in range(len(entries)):
            for j in range(i + 1, len(entries)):
                ...
                if outcome_a == outcome_b:
                    continue
                key = (call_repr, *sorted([f"{func_a}:{outcome_a}", f"{func_b}:{outcome_b}"]))
                if key in seen:
                    continue
                seen.add(key)
