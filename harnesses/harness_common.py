...
# Allow importing from harnesses/, pipeline/, and project root
...
def call_model(
    ...
    if not cmd_override and not model:
        raise ValueError(f"model must be specified (vendor={vendor!r})")
    if not cmd_override:
        if vendor == "alibaba":
        ...
        text, tokens_in, tokens_out = call_ollama(
            prompt, cmd_override=cmd_override, model=model
        )
        resolved_vendor = OLLAMA_VENDOR
...
def load_job(job_id: str) -> tuple[ClowderDB, dict]:
    ...
    jobs_cols = {
        row[1] for row in db.conn.execute("PRAGMA table_info(jobs)").fetchall()
    }
...
def complete_job(db: ClowderDB, job_id: str, output: str):
    ...
    db.conn.execute(
        ...
        WHERE job_id = ?
    """,
        (ts, ts, clean_job_output(output)[:2000], job_id),
    )
    db.conn.commit()
    db.close()
...
def spawn_retry_jobs(
    ...
    for job_id, agent_type, prompt, command in [
        (dev_id, "dev", dev_prompt, f"python -u harnesses/dev_harness.py {dev_id}"),
        (
            verify_id,
            "verifier",
            original_prompt,
            f"python -u harnesses/verify_harness.py {verify_id}",
        ),
    ]:
        db.conn.execute(
            """
    ...
    if current_job_id:
        ...
        if downstream:
            print(
                f"[verify] added {verify_id[:8]} as dependency for "
...
def spawn_child_dev(
    ...
    """,
        (
            ...
            dev_prompt,
            original_prompt,
            f"python -u harnesses/dev_harness.py {dev_id}",
            model,
            vendor,
            json.dumps([workspace_path]),
            parent_job_id,
