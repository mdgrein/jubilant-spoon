    ...
    def run(self, job_id: str) -> str:
        ...
        try:
            ...
            while True:
                ...
                # Print raw LLM output so it flows into job_output via server capture
                if result.get("raw_stdout"):
                    print(
                        f"[iter {result['iteration']}] LLM response:\n{result['raw_stdout'].strip()}",
                        flush=True,
                    )
...
Examples:
  ...
  python harness.py --prompt "Complex task" --max-iterations 100 --timeout 600
    ...
    parser.add_argument("job_id", nargs="?", help="Job ID to run (existing job mode)")
    parser.add_argument("--prompt", type=str, help="Job prompt (creates new job)")
    parser.add_argument(
        "--workspace", type=str, help="Workspace directory (default: current directory)"
    )
    parser.add_argument(
        "--max-iterations",
        type=int,
