...
def call_ollama(
    prompt: str, cmd_override: str | None = None, model: str | None = None
) -> tuple[str, int, int]:
    ...
    """
    if not model and not cmd_override:
        raise ValueError("model must be specified for local-ollama vendor")
    cmd = (
        cmd_override.split()
        if cmd_override
        else ["wsl", "ollama", "run", model, "--nowordwrap"]
    )
