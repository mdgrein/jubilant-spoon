...
class Agent:
    ...
    def _build_context(self) -> str:
        ...
        workspace_path = (
            str(self.tool_registry.allowed_paths[0])
            if self.tool_registry.allowed_paths
            else "."
        )
        ...
        context_parts = [
            ...
            "TRANSFORM OPS: uppercase, lowercase, title, strip",
            "",
            ...
            for entry in self.action_history[-3:]:
                ...
                if actions:
                    ...
                    context_parts.append(
                        f"  iter{entry['iteration']}: {','.join(tools_used)} -> {','.join(statuses)}"
                    )
            ...
            if isinstance(value, str):
                ...
                for match in matches:
                    ...
                    if 0 <= idx < len(results):
                        ...
                        # Otherwise do string replacement
                        value = value.replace(
                            f"{{{{actions[{idx}]}}}}", str(result_val)
                        )
                    else:
                        logger.warning(
                            f"Invalid action reference: actions[{idx}] (only {len(results)} actions executed)"
                        )
    ...
    def _execute_actions(self, llm_response: dict) -> list[dict]:
        ...
        for i, action in enumerate(validated.actions):
            logger.info(
                f"Executing action {i + 1}/{len(validated.actions)}: {action.tool}"
            )
