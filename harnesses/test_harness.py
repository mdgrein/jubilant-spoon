...
from dev_utils import detect_test_contradictions
...
def _extract_imported_function_signatures(
    test_file_path: Path, workspace: Path
) -> Dict[str, int]:
    """
    ...
    def visit_Call(self, node: ast.Call):
        ...
        if func_name and func_name in self.function_signatures:
            ...
            if actual_args < expected_args:
                ...
                expected_args == 0
                and actual_args > 0
                and func_name in ["fibonacci", "binary_sort", "convert_case"]
            ):  # Simple heuristic for functions that should take args
...
def _validate_test_function_calls(test_file_path: Path, workspace: Path) -> List[str]:
    ...
    try:
        ...
        errors.append(
            f"Could not parse imports from test file '{test_file_path.name}'."
        )
        return errors
    ...
    try:
        ...
        errors.append(
            f"Error validating function calls in '{test_file_path.name}': {e}"
        )
...
def main():
    ...
    for attempt in range(1, MAX_ITERATIONS + 1):
        print(f"\n\n{'=' * 80}")
        print(
            f"  ITERATION {attempt}/{MAX_ITERATIONS}  |  Job: {job_id[:8]}  |  File: tests/{test_filename}"
        )
        print(f"{'=' * 80}\n")
        ...
        if last_failure is None:
            ...
            model_prompt = _retry_prompt(
                module_name, task, existing_code, pytest_output
            )
        ...
        if not code.strip():
            ...
            llm_feedback = (
                "Your output is empty after cleaning. You must provide raw Python code."
            )
            ...
            try:
                ...
                print(
                    f"[test] LLM output is not valid Python code (parsing error): {e}"
                )
                llm_feedback = (
                    f"Your output must consist ONLY of valid raw Python code. "
        ...
        if ast_errors:
            print("[test] AST validation found issues:\n" + "\n".join(ast_errors))
            llm_feedback = (
        ...
        if not real_impl_exists:
            ...
            if stub_code:
                ...
                print(
                    "[test] WARNING: generate_stub returned empty (syntax error?); skipping stub"
                )
        else:
            print(
                f"[test] stub skipped — {stub_path.name} already exists (manual retry?)"
            )
        ...
        if returncode >= 2:
            ...
            print(
                f"[test] collection error (returncode {returncode}) — test file needs regeneration"
            )
            llm_feedback = (
    ...
    if not job_succeeded_in_loop:
        if last_failure == "format_error":
            ...
            fail_job(
                db,
                job_id,
                f"test validation failed (ruff or AST) after {MAX_ITERATIONS} attempts",
                tag="test",
            )
            ...
            fail_job(
                db,
                job_id,
                f"tests still failing after {MAX_ITERATIONS} attempts",
                tag="test",
            )
        else:
            fail_job(
                db,
                job_id,
                f"job failed after {MAX_ITERATIONS} attempts for unknown reason",
                tag="test",
            )
