...
def validate_llm_response(data: Any) -> LLMResponse:
    ...
    for i, action_raw in enumerate(actions_raw):
        if not isinstance(action_raw, dict):
            raise ValidationError(
                f"Action {i} must be dict, got {type(action_raw).__name__}"
            )
        ...
        if not isinstance(tool, str):
            raise ValidationError(
                f"Action {i} tool must be string, got {type(tool).__name__}"
            )

        if not isinstance(args, dict):
            raise ValidationError(
                f"Action {i} args must be dict, got {type(args).__name__}"
            )
        ...
        if unknown_action_fields:
            raise ValidationError(
                f"Action {i} has unknown fields: {unknown_action_fields}"
            )
