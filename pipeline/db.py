...
class ClowderDB:
    ...
    def __init__(self, db_path: str = "clowder.db"):
        ...
        self.db_path = db_path if is_uri else Path(db_path)
        ...
        # Enable WAL mode for better concurrency (not applicable to in-memory DBs)
        if not is_uri:
    ...
    def init_pipeline_schema(self):
        """Initialize pipeline database schema from schema_pipelines.sql."""
        schema_path = Path(__file__).parent / "schema_pipelines.sql"
        self.conn.executescript(schema_path.read_text(encoding="utf-8"))
        self.conn.commit()
