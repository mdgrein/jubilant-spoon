...
def parse_artifact_content(content: str, parse_strategy: str) -> list[str]:
    ...
    if parse_strategy == "json_array":
        try:
            ...
            print("Warning: Failed to parse as JSON, treating as single item")
            return [content]
...
def spawn_multiplied_jobs(
    ...
    source_type = multiplier_config.get(
    ...
    if source_type == "action":
        ...
        if not tasks_content:
            print(
                f"Warning: No tasks found in finish action for job {source_job_id[:8]}"
            )
            return []
    else:
        ...
        artifact = db_conn.execute(
            ...
            LIMIT 1
        """,
            (source_job_id, artifact_name),
        ).fetchone()

        if not artifact or not artifact["content"]:
            print(
                f"Warning: No artifact found for job {source_job_id[:8]}, name={artifact_name}"
            )
            return []
    ...
    if not items:
        print("Warning: Artifact parsing returned no items")
        return []
...
def check_and_spawn_multiplied_jobs(db_conn, completed_job_id: str, timestamp_fn):
    ...
    for template_job in template_jobs:
        ...
        """,
            (completed_job_id, template_job["template_job_id"]),
        ).fetchone()
        ...
        """,
            (pipeline_id, template_job["stage_order"]),
        ).fetchone()
