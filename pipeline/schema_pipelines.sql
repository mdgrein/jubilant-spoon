...
CREATE TABLE pipeline_templates (
    ...
    description TEXT,
    category TEXT,                                         -- display-only grouping (NULL = uncategorized)
    default_vendor TEXT NOT NULL DEFAULT 'local-ollama',  -- fallback vendor for all jobs in this template
    default_model TEXT,                                    -- fallback model (NULL = harness default)
...
CREATE TABLE template_jobs (
    ...
    chain_id TEXT,  -- Groups jobs into a named TDD chain (e.g. 'fibonacci', 'task'); NULL = standalone job
...
CREATE TABLE pipelines (
    ...
    schedule_id TEXT,  -- Set if this pipeline was spawned by a schedule
    FOREIGN KEY(template_id) REFERENCES pipeline_templates(template_id) ON DELETE SET NULL,
    FOREIGN KEY(schedule_id) REFERENCES pipeline_schedules(schedule_id) ON DELETE SET NULL
);
...
CREATE TABLE jobs (
    ...
    agent_type TEXT NOT NULL CHECK(agent_type IN ('planner', 'dev', 'tester', 'verifier', 'mock', 'command')),
    name TEXT NOT NULL DEFAULT '',  -- Human-readable job name (from template_jobs.name)
    ...
    chain_id TEXT,  -- TDD chain group (copied from template_jobs.chain_id; NULL = standalone)
...
-- =====================================================================
-- PIPELINE SCHEDULES: Cron-triggered pipeline instances
-- =====================================================================
CREATE TABLE IF NOT EXISTS pipeline_schedules (
    schedule_id    TEXT PRIMARY KEY,
    template_id    TEXT NOT NULL REFERENCES pipeline_templates(template_id) ON DELETE RESTRICT,
    name           TEXT NOT NULL,
    cron_expr      TEXT NOT NULL,        -- standard 5-field cron, e.g. "0 2 * * *"
    prompt         TEXT NOT NULL,
    workspace_path TEXT NOT NULL DEFAULT '/workspace',
    enabled        INTEGER NOT NULL DEFAULT 1,
    last_fired_at  TEXT,                 -- ISO8601, NULL = never fired
    next_fire_at   TEXT,                 -- pre-computed; indexed for efficient polling
    created_at     TEXT NOT NULL,
    updated_at     TEXT NOT NULL
);
