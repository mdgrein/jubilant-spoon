...
# Template to run. To switch vendor/model, edit default_vendor/default_model
# for this template_id in pipeline/seed_templates.sql and restart the server.
TEMPLATE_ID = "template-four-tasks"


def start_pipeline(prompt: str):
    """Start the four-tasks pipeline."""
    response = requests.post(
        f"{SERVER_URL}/pipelines/{TEMPLATE_ID}/start",
...
def main():
    ...
    print("\nView full results:")
    print(
        f"  python -c \"import sqlite3; conn = sqlite3.connect('clowder.db'); [print(dict(a)) for a in conn.execute('SELECT * FROM artifacts WHERE job_id IN (SELECT job_id FROM jobs WHERE pipeline_id = \\\"{pipeline_id}\\\")')]; conn.close()\""
    )
